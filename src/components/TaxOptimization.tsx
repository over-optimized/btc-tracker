...
const TaxOptimization: React.FC<TaxOptimizationProps> = ({
  ...
  const [hypotheticalPrice, setHypotheticalPrice] = useState<string>(currentPrice?.toString() || '');
  const [hypotheticalResult, setHypotheticalResult] = useState<any>(null);
  ...
  const getOptimizationIcon = (suggestion: string) => {
    if (suggestion.includes('Tax-loss harvesting')) return <TrendingDown className="text-red-500" />;
  ...
  const getBestMethod = () => {
    ...
    const validComparisons = methodComparison.filter((comp: any) => !comp.error);
    if (validComparisons.length === 0) return null;

    // Find method with highest gain (or lowest loss)
    return validComparisons.reduce((best: any, current: any) => {
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        {suggestions.length === 0 ? (
          <div className="text-center text-gray-600 py-8">
            ...
            <p>Upload transactions and set a current Bitcoin price to get optimization suggestions.</p>
          </div>
        ) : (
                ...
                {hypotheticalResult.error ? (
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{formatCurrency(hypotheticalResult.proceeds)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Cost Basis:</span>
                      <span className="font-medium">{formatCurrency(hypotheticalResult.costBasis)}</span>
                    </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {hypotheticalResult.holdingPeriod === HoldingPeriod.LONG_TERM ? 'Long-term' : 'Short-term'}
                      </span>
                    </div>
      ...
      {methodComparison && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {methodComparison.map((comparison: any, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-semibold text-gray-800 mb-3 mt-2">
                    {comparison.method}
                  </h3>
                  ...
                  {comparison.error ? (
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span className="text-gray-600">Cost Basis:</span>
                        <span className="font-medium">{formatCurrency(comparison.costBasis)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Holding Period:</span>
                        <span className={`px-2 py-1 text-xs rounded-full ${
          ...
          {getBestMethod() && (
            <div className="mt-4 p-3 bg-green-100 border border-green-200 rounded-lg">
              <p className="text-green-800 text-sm">
                <strong>Recommendation:</strong> The {getBestMethod().method} method provides the best 
                tax outcome for this hypothetical sale with a capital gain of{' '}
