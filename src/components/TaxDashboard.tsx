  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (!validation.isValid) {
        setError(
          `Tax calculation failed: ${validation.errors.map(e => e.message).join(', ')}`
        );
        setLoading(false);
        return;
      }
      ...
      setError(`Failed to generate tax report: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
        ...
        {transactions.length === 0 ? (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-yellow-800">No Transactions Found</span>
            </div>
            <p className="text-yellow-700 mt-1">
              Upload your transaction data first to generate tax reports. 
              Go to the Upload section to import your CSV files.
            </p>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-yellow-700 mt-1">
              No transactions found for the selected tax year. 
              You have {transactions.length} transactions total - 
              try selecting a different year or check your transaction dates.
            </p>
          </div>
        ) : (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-green-700 mt-1">
              Found {transactionsForYear.length} transactions for {configuration.taxYear}.
              Configure your settings below and generate your tax report.
            </p>
          </div>
        )}
        ...
        {report && !loading && (
          <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <span className="text-gray-600">Net Gains/Losses:</span>
                <span className={`ml-2 font-medium ${
                  report.summary.netGains >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  {report.summary.netGains >= 0 ? '+' : ''}
                  ${report.summary.netGains.toFixed(2)}
                </span>
              </div>
              <div>
                <span className="text-gray-600">Remaining BTC:</span>
                <span className="ml-2 font-medium">
                  ₿{report.summary.remainingBtc.toFixed(8)}
                </span>
              </div>
              <div>
